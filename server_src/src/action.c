...
/*   Updated: 2019/08/10 11:59:07 by gmorer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		action_nick(t_client **clients, t_client *client, t_response *resp)
{
	...
	ft_memcpy(old_nick, client->nick, sizeof(old_nick));
	...
	ft_bzero(client->nick, sizeof(client->nick));
	ft_memcpy(client->nick, new_nick, new_nick_len);
	ft_bzero(resp->buffer, sizeof(resp->buffer));
...
int		action_join(t_client **clients, t_client *client, t_response *resp)
{
	...
	ft_memcpy(client->channel, new_channel, new_channel_len);
	ft_bzero(resp->buffer, sizeof(resp->buffer));
	resp->message_length = snprintf(resp->buffer, sizeof(resp->buffer),
...
int		who_next(t_client **clis, t_client *cli, t_response *resp, char *chan)
{
	...
	{
		if (!ft_strncmp(chan, tmp->channel, ft_strlen(chan)))
		{
			if (off + 1 + ft_strlen(tmp->nick) > BUFFER_LEN)
				break ;
			resp->buffer[off + 1] = ' ';
			ft_memcpy(resp->buffer + off + 2, tmp->nick, ft_strlen(tmp->nick));
			off += 1 + ft_strlen(tmp->nick);
		}
		tmp = tmp->next;
	}
	if (off + 1 < BUFFER_LEN)
...
int		action_who(t_client **clients, t_client *cli, t_response *resp)
{
	...
	ft_bzero(channel_tmp, sizeof(channel_tmp));
	...
	if (!channel_len)
	{
		...
		ft_memcpy(channel_tmp, cli->channel, ft_strlen(cli->channel));
		channel_len = ft_strlen(channel_tmp);
	}
	else
	{
		if (channel_len > CHANNEL_NAME_LEN)
			return (send_to_client(cli, resp, ER_LG_NAM, sizeof(ER_LG_NAM)));
		ft_memcpy(channel_tmp, channel, channel_len);
	}
...
int		action_msg(t_client **clients, t_client *cli, t_response *resp)
{
	...
	if ((resp->activity = 0) || dest_len > NICK_LEN)
		return (send_to_client(cli, resp, ER_LG_NAM, sizeof(ER_LG_NAM)));
	while (tmp && (ft_strncmp(dest, tmp->nick, dest_len) || tmp == cli))
		tmp = tmp->next;
	ft_bzero(tmp ? buffer : resp->buffer, BUFFER_LEN);
	if (tmp)
		ft_memcpy(buffer, dest + dest_len + 1, resp->message_length -
				((dest - resp->buffer) + dest_len + 1));
	if (tmp)
		resp->message_length = snprintf(resp->buffer, sizeof(resp->buffer),
				"* From %s: %s", cli->nick, buffer);
	else
		ft_strcpy(resp->buffer, ERR_INV_NICK);
	if (!tmp)
